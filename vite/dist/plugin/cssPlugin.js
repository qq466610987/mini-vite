...
export const cssPlugin = (context) => {
    context.app.use(async (ctx, next) => {
        if (/\.css\??[^.]*$/.test(ctx.path)) {
            let cssRes = fs.readFileSync(path.join(context.basePath, ctx.path), "utf-8");
        ...
        if (/\.scss\??[^.]*$/.test(ctx.path)) {
            let scssRes = fs.readFileSync(path.join(context.basePath, ctx.path), "utf-8");
